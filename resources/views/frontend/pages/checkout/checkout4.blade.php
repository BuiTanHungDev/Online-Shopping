                            ...
                            <table class="table mb-0">
                                <tbody>
                                    @php
                                        ...
                                        $deliveryCharge = floatval(\Illuminate\Support\Facades\Session::get('checkout')[0]['delivery_charge'] ?? 0);
                                        $total = $subtotal + $deliveryCharge - $couponValue;
                                    @endphp
                                    ...
                                    @if (session()->has('coupon'))
                                        <tr>
                                            <td>Coupon :</td>
                                            <td>${{ number_format($couponValue, 2) }}</td>
                                        </tr>
                                    @endif
                                    ...
                                    <tr>
                                        <td>Shipping</td>
                                        <td>${{ number_format(\Illuminate\Support\Facades\Session::get('checkout')[0]['delivery_charge'], 2) }}
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        @elseif (\Illuminate\Support\Facades\Session::has('checkout'))
                                            ...
                                            <td>${{ number_format((float) str_replace(',', '', \Gloudemans\Shoppingcart\Facades\Cart::subtotal()) + (float) \Illuminate\Support\Facades\Session::get('checkout')[0]['delivery_charge'], 2) }}
                                            </td>
                                            @elseif (\Illuminate\Support\Facades\Session::has('coupon') && \Illuminate\Support\Facades\Session::has('checkout'))
                                            <td>
                                                ${{ number_format((float) str_replace(',', '', \Gloudemans\Shoppingcart\Facades\Cart::subtotal()) + (float) \Illuminate\Support\Facades\Session::get('checkout')[0]['delivery_charge'] - (float)\Illuminate\Support\Facades\Session::get('coupon')['value'], 2) }}
                                            </td>
                                        @endif
                                    </tr> --}}
                                </tbody>
